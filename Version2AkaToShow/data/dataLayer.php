<?php
	...
	function getProyectsByCategory($category){
		...
		if ($conn) {
			...
			$prepared_stmt = $conn -> prepare("SELECT proyect.id, proyect.name, proyect.description
					  FROM proyect
					  WHERE proyect.category = '$category' AND proyect.isBeforeAfter = '0'");
			...
			if ($prepared_stmt -> execute()) {
			    ...
			    while($row = mysqli_fetch_assoc($result)) {
			    	...
			    	$arrOfPicsOfProyect = getImagesOfCurrentProyect($row["id"]);
			        ...
			        $currentRow = array("proyectId" => $row["id"], "proyectName" => $row["name"], "proyectDescription" => $row["description"], "proyectCategory" => $category, "pics" => $arrOfPicsOfProyect);
	...
	function getProyectsByCategoryBeforeAfter($category){
		...
		if ($conn) {
			...
			$prepared_stmt = $conn -> prepare("SELECT proyect.id, proyect.name, proyect.description
					  FROM proyect
					  WHERE proyect.category = '$category' AND proyect.isBeforeAfter = '1'");
		...
		if ($NewCon){
			...
			if ($picStmt -> execute()){
			    ...
			    while($PicRow = mysqli_fetch_assoc($picsResult)){
			        $picsCurrentRow = array('picId' => $PicRow["id"], 'picType' => $PicRow["type"], 'picLink' => $PicRow["linkToPic"]);
	...
	function addProduct( $name, $description, $price, $category){
		...
		if ($conn) 
		{
			$query = "INSERT INTO product ( pName, pDescription, pPrice, category) 
            		  VALUES  ( ?,?,?,?)";
			$prepared_stmt = $conn -> prepare ( $query );
			$prepared_stmt -> bind_param ( 'ssss' , $name, $description, $price, $category);
	...
	function addToCart($idc, $idp){
		...
		if ($conn) 
		{
			$prepared_stmt = $conn -> prepare ( "INSERT INTO shoppingcart ( idclient, idproduct ) 
            		  VALUES  ( $idc, $idp)");
	...
	function removeFromCart($idc, $idp){
		...
		if ($conn) 
		{
			$prepared_stmt = $conn -> prepare ( "DELETE FROM shoppingcart
            		  WHERE  ( idclient = '$idc' AND idproduct = '$idp');");
			if  ( $prepared_stmt -> execute() ) 
			{
				$conn -> close();
				return array('status' => "success", "code" => 200);
	...
	function getCartItems($idc)
	{
		...
		if ($conn){
			...
			$prepared_stmt = $conn -> prepare("SELECT id, pName, pDescription, pPrice, pPicture
					  FROM product INNER JOIN shoppingcart ON id = idproduct
					  WHERE idclient = $idc;");
			if ($prepared_stmt -> execute()){
				$result = $prepared_stmt->get_result();
			    $arrOfProducts = array();
			    while($row = mysqli_fetch_assoc($result)){
			        $currentRow = array("productId" => $row["id"], "productName" => $row["pName"], "productDescription" => $row["pDescription"], "productPrice" => $row["pPrice"], "linkToPic" => $row["pPicture"]);
			        array_push($arrOfProducts, $currentRow);
			    }
			    $conn -> close();
			    return array('status' => "success", "code" => 200, 'response' => $arrOfProducts);
	...
	function getTotalFromCart($idc)
	{
		...
		if ($conn){
			...
			$prepared_stmt = $conn -> prepare("SELECT SUM(product.pPrice) as productTotal
					  FROM product INNER JOIN shoppingcart ON product.id = shoppingcart.idproduct
					  WHERE shoppingcart.idclient = $idc;");
			if ($prepared_stmt -> execute()){
				$result = $prepared_stmt->get_result();
			    $arrOfProducts = array();
			    while($row = mysqli_fetch_assoc($result)){
	...
	function getListOfItems($name)
	{
		$conn = connect();
		if ($conn){
			$prepared_stmt = $conn -> prepare("SELECT id, pName, pDescription, pPrice, pPicture, category FROM product
					  WHERE (`pName` LIKE '%".$name."%');");
			if ($prepared_stmt -> execute()){
				$result = $prepared_stmt->get_result();
			    $arrOfProducts = array();
			    while($row = mysqli_fetch_assoc($result)){
			         $currentRow = array("productId" => $row["id"], "productName" => $row["pName"], "productDescription" => $row["pDescription"], "productCategory" => $row["category"], "productPrice" => $row["pPrice"], 'linkToPic' => $row["pPicture"]);
			        array_push($arrOfProducts, $currentRow);
			    }
			    $conn -> close();
			    return array('status' => "success", "code" => 200, 'response' => $arrOfProducts);
